...
public class PathsControllerTests : TestBase
{
  ...
  {
    samplePath = new Path { Id = Guid.NewGuid(), Title = "Path1", Description = "Description1" };
    paths = new List<Path>
    {
      samplePath,
      new Path { Id = Guid.NewGuid(), Title = "Path2", Description = "Description2" },
      new Path { Id = Guid.NewGuid(), Title = "Path3", Description = "Description3" },
      new Path { Id = Guid.NewGuid(), Title = "Path4", Description = "Description4" },
      new Path { Id = Guid.NewGuid(), Title = "Path5", Description = "Description5" }
    };
  ...
  public async Task Get_ReturnsPath()
  {
    ...
    var result = await controller.Get(samplePath.Id);
    var content = GetObjectResultContent<Path>(result.Result);

    Assert.IsInstanceOf(typeof(OkObjectResult), result.Result);
    Assert.IsNotNull(content);
    Assert.AreEqual(samplePath.Id, content.Id);
  }
  ...
  {
    var updateCommand = new UpdatePath { Id = samplePath.Id, Title = "Update title", Description = "Update Description" };
    var controller = new PathsController(moqMediator.Object);

    var result = await controller.Update(samplePath.Id, updateCommand);
    var content = GetObjectResultContent<Path>(result.Result);
  ...
  public async Task Update_ReturnsBadRequest_WhenRequestedIdDoesNotMatchCommandId()
  {
    ...
    var result = await controller.Update(Guid.NewGuid(), updateCommand);
  ...
  public async Task Delete_ReturnsNoContent()
  {
    ...
    var result = await controller.Delete(samplePath.Id);
