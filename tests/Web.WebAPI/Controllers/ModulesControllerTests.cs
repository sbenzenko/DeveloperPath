...
public class ModulesControllerTests : TestBase
{
  ...
  {
    sampleModule = new Module { Id = Guid.NewGuid(), Title = "Module1", Description = "Description1" };
    modules = new List<Module>
    {
      sampleModule,
      new Module { Id = Guid.NewGuid(), Title = "Module2", Description = "Description2" }
    };
  ...
  public async Task Create_ReturnsCreatedAtRoute()
  {
    ...
    Assert.IsNotNull(content);
    Assert.AreEqual(sampleModule.Id, content.Id);
  }

  [Test]
  public async Task Update_ReturnsUpdatedModule_WhenRequestedIdMatchesCommandId()
  {
    var updateCommand = new UpdateModule { Id = sampleModule.Id, Order = 0, Title = "Update title", Description = "Update Description" };
    var controller = new ModulesController(moqMediator.Object);

    var result = await controller.Update(sampleModule.Id, updateCommand);
    var content = GetObjectResultContent<Module>(result.Result);
  ...
  public async Task Delete_ReturnsNoContent()
  {
    ...
    var result = await controller.Delete(Guid.NewGuid(), sampleModule.Id);
