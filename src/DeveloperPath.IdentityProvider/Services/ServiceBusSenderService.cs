...
public class ServiceBusSenderService(string connectionString) : IServiceBusSenderService
{
  private const string VerificationEmailQueue = "email-verification-queue";

  private readonly ServiceBusClient _serviceBusClient = new(connectionString ?? throw new ArgumentNullException(nameof(connectionString)));
