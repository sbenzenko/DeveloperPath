...
namespace DeveloperPath.WebApi.Controllers
{
  ...
  public class ModulesController : ApiController
  {
    ...
    public async Task<ActionResult<Response<IEnumerable<ModuleDto>>>> Get(int pathId, [FromQuery] PaginationFilter filter = null)
    {
      if (filter == null)
      {
        IEnumerable<ModuleDto> result = await Mediator.Send(new GetModuleListQuery { PathId = pathId });
        return Ok(result);
      }
      else
      {
    ...
    {
      ModuleDto result = await Mediator.Send(new GetModuleQuery { PathId = pathId, Id = moduleId });
      ...
      [FromBody] CreateModuleCommand command)
    {
      ...
      ModuleDto result = await Mediator.Send(command);

      return CreatedAtRoute("GetModule", new { pathId = command.PathId, moduleId = result.Id }, result);
    }
