...
public class SourcesController : ApiController
{
  ...
  [HttpGet]
  [HttpHead]
  public async Task<ActionResult<IEnumerable<Source>>> Get(Guid moduleId, Guid themeId, CancellationToken ct = default)
  {
    IEnumerable<Source> sources = await Mediator.Send(
        new GetSourceListQuery { ModuleId = moduleId, ThemeId = themeId }, ct);
  ...
  public async Task<ActionResult<Source>> Get(Guid moduleId, Guid themeId, Guid sourceId, CancellationToken ct = default)
  {
    Source model = await Mediator.Send(
        new GetSourceQuery { ModuleId = moduleId, ThemeId = themeId, Id = sourceId }, ct);
  ...
  public async Task<ActionResult<Source>> Create(Guid moduleId, Guid themeId, [FromBody] CreateSource command, CancellationToken ct = default)
  {
    if (moduleId != command.ModuleId || themeId != command.ThemeId)
      return BadRequest();

    Source model = await Mediator.Send(command, ct);
  ...
  [Consumes("application/json")]
  public async Task<ActionResult<Source>> Update(Guid moduleId, Guid themeId, Guid sourceId, [FromBody] UpdateSource command, CancellationToken ct = default)
  {
    if (moduleId != command.ModuleId || themeId != command.ThemeId || sourceId != command.Id)
      return BadRequest();
  ...
  public async Task<ActionResult> Delete(Guid moduleId, Guid themeId, Guid sourceId, CancellationToken ct = default)
  {
    await Mediator.Send(new DeleteSource
    { ModuleId = moduleId, ThemeId = themeId, Id = sourceId }, ct);
