...
[Route("api/modules/{moduleId:guid}/themes")]
public class ThemesController : ApiController
{
    ...
    [HttpGet]
    [HttpHead]
    public async Task<ActionResult<IEnumerable<Theme>>> Get(Guid moduleId, CancellationToken ct = default)
    {
        IEnumerable<Theme> themes = await Mediator.Send(
            new GetThemeListQuery { ModuleId = moduleId }, ct);
    ...
    [HttpGet("{themeId:guid}", Name = "GetTheme")]
    [HttpHead("{themeId:guid}")]
    public async Task<ActionResult<Theme>> Get(Guid moduleId, Guid themeId, CancellationToken ct = default)
    {
        var model = await Mediator.Send(
            new GetThemeQuery { ModuleId = moduleId, Id = themeId }, ct);
    ...
    public async Task<ActionResult<Theme>> Create(Guid moduleId, [FromBody] CreateTheme command)
    {
        if (moduleId != command.ModuleId)
            return BadRequest();

        Theme model = await Mediator.Send(command);

        return CreatedAtRoute("GetTheme",
            new { moduleId = model.ModuleId, themeId = model.Id }, model);
    }
    ...
    [Consumes("application/json")]
    public async Task<ActionResult<Theme>> Update(Guid moduleId, Guid themeId, [FromBody] UpdateTheme command)
    {
        if (moduleId != command.ModuleId || themeId != command.Id)
            return BadRequest();
    ...
    public async Task<ActionResult> Delete(Guid moduleId, Guid themeId)
    {
        await Mediator.Send(new DeleteTheme { ModuleId = moduleId, Id = themeId });
