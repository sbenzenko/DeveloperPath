...
public class ApplicationDbContext : DbContext, IApplicationDbContext
{
  ...
  protected override void OnModelCreating(ModelBuilder builder)
  {
    ...
    // Apply soft deletion filter to all queries to AuditableEntity
    builder.ApplyConfigurationsFromAssembly(Assembly.GetExecutingAssembly());
    var entities = builder.Model
        .GetEntityTypes()
        .Where(e => e.ClrType.BaseType == typeof(AuditableEntity))
        .Select(e => e.ClrType);

    Expression<Func<AuditableEntity, bool>> expression = deletion => deletion.Deleted == null;
